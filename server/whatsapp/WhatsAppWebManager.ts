...
const pendingReconnects = new Set<number>(); // Track ongoing reconnect attempts to prevent race condition
...
function scheduleReconnect(tenantId: number): void {
  ...
  // Prevent race condition: don't schedule if reconnect already pending
  ...
  setTimeout(async () => {
    try {
      ...
      // Mark as pending and attempt reconnect
      ...
      pendingReconnects.delete(tenantId);
    }
  }, delay);
}
