...
export const knowledgeBaseRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      const conditions: (SQL | undefined)[] = [eq(knowledgeBase.tenantId, ctx.user.userId)];
  ...
  resync: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (article.type !== "link" || !article.sourceUrl) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Only URL articles can be resynced",
        });
      }
