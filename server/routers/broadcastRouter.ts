  ...
  getAudience: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.filter === "all") {
        ...
        return { count: allCustomers.length, customers: allCustomers };
      }

      const now = new Date();
      const cutoff = new Date(now.getTime() - (input.filter === "active_7d" ? 7 : 30) * 24 * 60 * 60 * 1000);

      // Fixed: Single SQL query with EXISTS subquery instead of N+1 loop
        ...
        .where(
          and(
            ...
            sql`EXISTS (
              ...
              AND ${conversations.phoneNumber} = ${customers.phoneNumber}
              AND ${conversations.createdAt} >= ${cutoff}
              LIMIT 1
            )`
          )
        );
        ...
        try {
          for (const customer of targetCustomers) {
            try {
              ...
              console.error(`❌ Failed to send broadcast to ${customer.phoneNumber}:`, err instanceof Error ? err.message : String(err));
              failed++;
            }
          ...
          console.error(`❌ Broadcast error for tenant ${tenantId}:`, err instanceof Error ? err.message : String(err));
        }
      })();
