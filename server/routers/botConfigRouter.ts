...
import { botConfig, systemSettings } from "../../drizzle/schema.js";
...
export const botConfigRouter = router({
  get: protectedProcedure.query(async ({ ctx }) => {
    ...
    // ─── Check for global AI model override ───────────────────────────────
    let aiModel = config.aiModel;
    let aiApiUrl = config.aiApiUrl;
    let aiApiKey = decrypt(config.aiApiKey || "");
    let aiTemperature = config.aiTemperature.toString();
    ...
    try {
      ...
      // Apply global overrides if globalAiModel is set (non-empty string)
      if (globalSettings?.globalAiModel) {
        aiModel = globalSettings.globalAiModel;
        aiApiUrl = globalSettings.globalAiApiUrl;
        aiApiKey = globalSettings.globalAiApiKey;
        aiTemperature = globalSettings.globalAiTemperature.toString();
        console.log(`ℹ️  Tenant ${ctx.user.userId} using global AI model: ${aiModel}`);
      }
    } catch (err: any) {
    ...
    return {
      ...
      // Dual AI (Groq + Ollama)
      fallbackAiEnabled: config.fallbackAiEnabled,
      fallbackAiModel: config.fallbackAiModel,
      fallbackAiUrl: config.fallbackAiUrl,
      fallbackAiKey: decrypt(config.fallbackAiKey || ""),
      fallbackAiTimeout: config.fallbackAiTimeout,
      usedFallbackCount: config.usedFallbackCount,
      lastFallbackAt: config.lastFallbackAt,
      // Birthday messages
  ...
  update: protectedProcedure
    .input(z.object({
      ...
      // Dual AI (Groq + Ollama)
      fallbackAiEnabled: z.boolean().optional(),
      fallbackAiModel: z.string().max(100).optional(),
      fallbackAiUrl: z.string().max(255).optional(),
      fallbackAiKey: z.string().optional(),
      fallbackAiTimeout: z.number().min(1000).max(30000).optional(),
      // Birthday messages
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Dual AI (Groq + Ollama)
      if (input.fallbackAiEnabled !== undefined) colUpdates.fallbackAiEnabled = input.fallbackAiEnabled;
      if (input.fallbackAiModel !== undefined) colUpdates.fallbackAiModel = input.fallbackAiModel;
      if (input.fallbackAiUrl !== undefined) colUpdates.fallbackAiUrl = input.fallbackAiUrl;
      if (input.fallbackAiKey !== undefined) colUpdates.fallbackAiKey = encryptIfNeeded(input.fallbackAiKey);
      if (input.fallbackAiTimeout !== undefined) colUpdates.fallbackAiTimeout = input.fallbackAiTimeout;
