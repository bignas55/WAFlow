...
import { customers, conversations, appointments } from "../../drizzle/schema.js";
...
export const crmRouter = router({
  customers: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (offset > MAX_OFFSET) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: `Pagination offset exceeds maximum (${MAX_OFFSET}). Use a smaller page number or limit.`,
        });
      }
      const conditions: (SQL | undefined)[] = [eq(customers.tenantId, tenantId)];
  ...
  addTag: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const tenantId = ctx.user.userId;
      const [customer] = await db.select({ id: customers.id, tenantId: customers.tenantId, tags: customers.tags }).from(customers)
      ...
      // Verify tenant isolation before mutation
