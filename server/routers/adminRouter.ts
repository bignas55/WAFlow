...
import { users, botConfig, conversations, templates, knowledgeBase, customers, systemSettings } from "../../drizzle/schema.js";
...
export const adminRouter = router({
  ...
  disconnectTenant: adminProcedure
    .input(z.object({ tenantId: z.number().int().positive("tenantId must be a positive integer") }))
  ...
  updateTenantConfig: adminProcedure
    .input(z.object({
      tenantId: z.number().int().positive("tenantId must be a positive integer"),
  ...
  deleteTenantKBEntry: adminProcedure
    .input(z.object({ id: z.number().int().positive("id must be a positive integer") }))
    .mutation(async ({ input }) => {
...
${JSON.stringify(snapshot, null, 2)}`;
    ...
    const openai = new OpenAI({ baseURL: aiCfg.aiApiUrl, apiKey: decrypt(aiCfg.aiApiKey || "") || "ollama", timeout: 60000 });
...
${input.systemContext ? `\nLAST DIAGNOSIS CONTEXT:\n${input.systemContext}` : ""}${tenantCtx}`;
  ...
  executeAIFix: adminProcedure
    .input(z.object({
      ...
      tenantId: z.number().int().positive("tenantId must be a positive integer").optional(),
    }))
    .mutation(async ({ input }) => {
...
Respond in the same language the customer uses.`;
  ...
  addKBFromUrl: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: `Could not fetch URL: ${err.message}`,
        });
      }
      ...
      if (content.length < 50) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Could not extract meaningful content from this URL. The page may require JavaScript or block crawlers.",
        });
      }
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Update global AI model for all tenants
  // ──────────────────────────────────────────────────────────────────────────
  updateGlobalAiModel: adminProcedure
    .input(z.object({
      aiModel: z.string().min(1, "AI model name required").max(100),
      aiApiUrl: z.string().min(1, "API URL required").max(255),
      aiApiKey: z.string().min(1, "API key required"),
      aiTemperature: z.number().min(0).max(2).default(0.7),
    }))
    .mutation(async ({ input, ctx }) => {
      try {
        // Upsert global settings (insert or update if exists)
        await db.insert(systemSettings)
          .values({
            key: "global_ai",
            globalAiModel: input.aiModel,
            globalAiApiUrl: input.aiApiUrl,
            globalAiApiKey: input.aiApiKey,
            globalAiTemperature: input.aiTemperature,
            lastUpdatedBy: ctx.user.userId,
        ...
        // Broadcast to all connected clients in dashboard room
        io.to("dashboard").emit("system:aiModelChanged", {
          aiModel: input.aiModel,
          aiApiUrl: input.aiApiUrl,
          aiTemperature: input.aiTemperature,
          changedBy: ctx.user.email,
          changedAt: new Date(),
        });

        console.log(`✅ Admin ${ctx.user.email} updated global AI model to ${input.aiModel}`);
        return {
          success: true,
          message: "Global AI model updated for all tenants",
          affectedTenants: "all",
        };
      } catch (error) {
        console.error("❌ Error updating global AI model:", error);
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "Failed to update global AI model",
        });
      }
    }),

  // Get current global AI settings
  getGlobalAiModel: adminProcedure.query(async () => {
    try {
      const settings = await db.query.systemSettings.findFirst({
        where: eq(systemSettings.key, "global_ai"),
      });

      if (!settings || !settings.globalAiModel) {
        return { isSet: false, settings: null };
      }

      return {
        isSet: true,
        settings: {
          aiModel: settings.globalAiModel,
          aiApiUrl: settings.globalAiApiUrl,
          aiTemperature: settings.globalAiTemperature,
      ...
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Failed to fetch global AI model",
      });
    }
  }),
});
