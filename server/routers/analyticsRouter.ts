...
export const analyticsRouter = router({
  ...
  overview: protectedProcedure.query(async ({ ctx }) => {
    ...
    const appts = await db.select({ status: appointments.status, customerId: appointments.customerId })
      .from(appointments);
    // Filter by tenant via customers
    const tenantCustomerIds = (await db.select({ id: customers.id }).from(customers).where(eq(customers.tenantId, tenantId))).map(c => c.id);
    const tenantAppts = appts.filter(a => tenantCustomerIds.includes(a.customerId));
    ...
    return {
      ...
      totalAppointments: tenantAppts.length,
      completedAppointments: tenantAppts.filter(a => a.status === "completed").length,
      cancelledAppointments: tenantAppts.filter(a => a.status === "cancelled").length,
      noShowAppointments: tenantAppts.filter(a => a.status === "no_show").length,
    ...
    .query(async ({ ctx, input }) => {
      ...
      const rows = await db
        ...
        .where(and(
          ...
          sql`${appointments.customerId} IN (${tenantCustomerIds.join(",")})`
        ))
  ...
  revenue: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const tenantCustomerIds = (await db.select({ id: customers.id }).from(customers)
        .where(eq(customers.tenantId, tenantId))).map(c => c.id);
      if (!tenantCustomerIds.length) return { total: 0, perDay: [] };
  ...
  peakHours: protectedProcedure.query(async ({ ctx }) => {
    ...
    const rows = await db.execute(
      sql`SELECT
            DAYOFWEEK(STR_TO_DATE(CONCAT(${appointments.date}, ' ', ${appointments.time}), '%Y-%m-%d %H:%i')) AS dow,
            FLOOR(CAST(SUBSTRING(${appointments.time}, 1, 2) AS UNSIGNED) / 2) * 2 AS hour_bucket,
            COUNT(*) AS cnt
          FROM appointments
          WHERE date >= ${sinceStr}
            AND customer_id IN (${sql.raw(tenantCustomerIds.join(","))})
