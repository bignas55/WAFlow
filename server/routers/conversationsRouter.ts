...
import { conversations, botConfig, customers, templates } from "../../drizzle/schema.js";
...
export const conversationsRouter = router({
  ...
  stats: protectedProcedure.query(async ({ ctx }) => {
    ...
    const todayPhones = await db
      .select({ phone: conversations.phoneNumber, firstSeen: sql<string>`MIN(created_at)` })
      .from(conversations)
      .where(eq(conversations.tenantId, ctx.user.userId))
      .groupBy(conversations.phoneNumber)
    ...
    return {
      ...
      satisfactionScore: null,   // no survey system yet — hide rather than fake
