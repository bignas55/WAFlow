  ...
  getAvailableDates: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const holidaySet = new Set<string>(
        (await db.select({ date: holidays.date }).from(holidays)).map(h => h.date)
      );
  ...
  getAvailableSlots: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const slots = await db.select().from(availableSlots)
        .where(and(eq(availableSlots.dayOfWeek, jsDay), eq(availableSlots.isActive, true)));
      ...
      const booked = await db
        ...
        .where(and(
          sql`${customers.tenantId} = ${config.tenantId}`,
