...
function createRateLimiter(options: {
  ...
  skipPaths?: string[]; // paths to skip (exact prefix match)
}) {
  ...
  setInterval(() => {
    ...
    for (const [key, entry] of store.entries()) {
      if (now - entry.windowStart > options.windowMs * 2) store.delete(key);
    }
  }, 5 * 60 * 1000);
...
/** WhatsApp webhook limiter: 5000 requests / min (from Meta) — scaled for high volume */
...
/** Live receptionist limiter: 30 messages / min per IP (public endpoint) */
export const receptionistLimiter = createRateLimiter({
  windowMs: 60 * 1000,
  max:      30,
  message:  "Too many messages. Please wait a moment before continuing.",
});

/** Admin limiter: 100 requests / min (for admin-only operations) */
 ...
 */
const twoFaAttempts = new Map<string, { count: number; windowStart: number }>();
setInterval(() => {
  const now = Date.now();
  for (const [key, entry] of twoFaAttempts.entries()) {
    if (now - entry.windowStart > 15 * 60 * 1000 * 2) twoFaAttempts.delete(key);
  }
}, 5 * 60 * 1000);

export function checkTwoFaRateLimit(ip: string): { allowed: boolean; retryAfterSeconds: number } {
...
export function resetTwoFaRateLimit(ip: string): void {
...
const phoneMsgStore = new Map<string, WindowEntry>();
setInterval(() => {
  const now = Date.now();
  for (const [key, entry] of phoneMsgStore.entries()) {
    if (now - entry.windowStart > 120_000) phoneMsgStore.delete(key);
  }
}, 60_000);
 ...
 */
export function checkPhoneMessageLimit(
  tenantId: number,
  phone: string,
  windowMs = 60_000,
  max = 20,
...
const failedLogins = new Map<string, { count: number; firstSeen: number; lastSeen: number }>();
setInterval(() => {
  const cutoff = Date.now() - 60 * 60 * 1000; // purge entries older than 1 hour
  for (const [key, entry] of failedLogins.entries()) {
    if (entry.lastSeen < cutoff) failedLogins.delete(key);
  }
}, 10 * 60 * 1000);
...
export function getFailedLoginCount(ip: string): number {
