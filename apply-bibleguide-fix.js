#!/usr/bin/env node
/**
 * Direct Database Fix for BibleGuide Bot
 * This connects to your MySQL database and applies all necessary updates
 */
...
async function applyFix() {
  ...
  try {
    ...
    const [tenants] = await connection.execute('SELECT DISTINCT tenantId FROM botConfig ORDER BY tenantId');
    ...
    tenants.forEach((t, i) => console.log(`  ${i + 1}. Tenant ID: ${t.tenantId}`));

    // Use first tenant or ID 1
    const TENANT_ID = tenants[0]?.tenantId || 1;
    ...
    await connection.execute('DELETE FROM botMenuOptions WHERE tenantId = ?', [TENANT_ID]);
    ...
    const insertQuery = `
      ...
      (tenantId, itemNumber, title, description, actionType, response, sortOrder, isActive)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;
    ...
    const [config] = await connection.execute('SELECT enableMenuMode, menuGreeting, businessName FROM botConfig WHERE tenantId = ?', [TENANT_ID]);
    const [items] = await connection.execute('SELECT itemNumber, title, description FROM botMenuOptions WHERE tenantId = ? ORDER BY sortOrder', [TENANT_ID]);

    console.log(`\n✓ Bot Config:`);
    console.log(`  - Menu Mode Enabled: ${config[0]?.enableMenuMode === 1 ? '✅ YES' : '❌ NO'}`);
    console.log(`  - Business Name: ${config[0]?.businessName}`);
    console.log(`  - Greeting starts with: "${config[0]?.menuGreeting?.substring(0, 40)}..."`);
    ...
    console.log(`\n🎉 SUCCESS! BibleGuide bot is now configured!\n`);
    console.log(`📝 NEXT STEPS:`);
    console.log(`  1. Restart your bot application`);
    console.log(`  2. Send "hello" or "hi" to the bot`);
    console.log(`  3. You should see the BibleGuide age group greeting`);
    console.log(`  4. Select an age group (1-5) to test\n`);
    ...
    console.error('  • Verify MySQL/MariaDB is accessible at localhost:3306\n');
