...
import {
  ...
  customers, escalationRules, spamLogs, rateLimits, users, appointments, services,
...
export async function processWhatsAppWebhook(msg: WebhookMessage): Promise<void> {
  ...
  try {
    ...
    const isBibleBot = config.systemPrompt?.includes("Bible") || config.systemPrompt?.includes("bible") || config.systemPrompt?.includes("📖");
    ...
    const isPersonalMessage = isBibleBot && (
    ...
    if (isBibleBot && /\bquiz\s*time\b/i.test(msg.messageText)) {
    ...
    // ── Help / command list ───────────────────────────────────────────────────
    if (isBibleBot && /^(help|commands|what can you do|features|all features)[\s!?]*$/i.test(msg.messageText.trim())) {
      const helpMsg =
        `📋 *All Features*\n\n` +
        `*📖 Bible Study*\n` +
    ...
    // AI fallback (Ollama or OpenAI-compatible)
    ...
    if (aiResult) {
      ...
      await saveConversation(msg.tenantId, { ...msg, response: aiResult.response, source: "ai", language: aiResult.language, sentiment: aiResult.sentiment, responseTimeMs: Date.now() - startTime });
      await incrementMessageUsage(msg.tenantId);
      ...
      if (isBibleBot && !hasFormattedVerse && !isPersonalMessage) {
...
async function generateAIResponse(
  ...
  sessionTopic = "",
): Promise<{ response: string; language: string; sentiment: string } | null> {
  try {
    ...
    const systemPrompt =
      ...
      config.systemPrompt.replace("{businessName}", config.businessName) +
    ...
    try {
      completion = await openai.chat.completions.create(createParams as any);
      ...
      // ── Layer 2: context-exceeded retry with minimal history ──────────────
      // If Ollama rejects due to context overflow, strip down to just the last
      // 2 exchanges (4 messages) and try once more before touching the fallback.
      if (isContextErr && messages.length > 3) {
        console.warn(`⚠️  [Tenant ${msg.tenantId}] Context limit exceeded — retrying with minimal history (last 2 exchanges)`);
        const userAndAssistant = messages.slice(1, -1); // everything between system & current user msg
        const lastFour = userAndAssistant.slice(-4);    // at most 2 user+assistant pairs
        const minimalMessages: OpenAI.ChatCompletionMessageParam[] = [
          messages[0],       // system prompt (always kept)
          ...lastFour,
          messages[messages.length - 1], // current user message
        ];
        try {
          ...
          console.log(`✅ [Tenant ${msg.tenantId}] Minimal-history retry succeeded`);
        } catch (retryErr: any) {
          // Minimal retry also failed — fall through to the configured fallback model
          console.warn(`⚠️  Minimal-history retry also failed (${retryErr.message}) — trying fallback model`);
          const fbUrl   = config.aiFallbackApiUrl?.trim();
          const fbKey   = decrypt(config.aiFallbackApiKey?.trim() || "") || "ollama";
          const fbModel = config.aiFallbackModel?.trim();
          if (fbUrl && fbModel) {
            const fallbackClient = new OpenAI({ baseURL: fbUrl, apiKey: fbKey });
            const isFbOllama = fbUrl.includes("11434") || fbUrl.toLowerCase().includes("ollama");
            completion = await fallbackClient.chat.completions.create({
              model: fbModel,
              messages: minimalMessages,
              temperature: parseFloat(config.aiTemperature?.toString() || "0.7"),
              max_tokens: config.maxTokens,
              ...(isFbOllama ? { options: { num_ctx: OLLAMA_NUM_CTX } } : {}),
            } as any);
          } else {
            throw retryErr;
          }
        ...
        // ── Layer 3: configured fallback model (non-context errors) ──────────
        const fbUrl   = config.aiFallbackApiUrl?.trim();
        const fbKey   = decrypt(config.aiFallbackApiKey?.trim() || "") || "ollama";
        const fbModel = config.aiFallbackModel?.trim();
        if (fbUrl && fbModel) {
          console.warn(`⚠️  Primary AI failed (${primaryErr.message}) — retrying with fallback model ${fbModel}`);
          const fallbackClient = new OpenAI({ baseURL: fbUrl, apiKey: fbKey });
          const isFbOllama = fbUrl.includes("11434") || fbUrl.toLowerCase().includes("ollama");
          completion = await fallbackClient.chat.completions.create({
            model: fbModel,
    ...
    return { response, language, sentiment };
  } catch (error: any) {
