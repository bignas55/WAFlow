...
const INJECTION_PATTERNS: RegExp[] = [
  ...
  /forget\s+(?:all\s+)?(?:your\s+)?(?:previous|prior|above|earlier)\s+(?:instructions?|prompts?|rules?|context|role)/i,

  // "You are now / act as" jailbreak attempts
  /\byou\s+are\s+now\b(?!\s+(?:available|open|here|ready|my\b))/i,  // not "you are now available" / "… my favourite …"
...
export interface SanitizeResult {
  /** The sanitized message text, safe to pass to the AI. */
  text: string;
  /** True if injection patterns were detected. */
  injectionDetected: boolean;
  /** True if the message was truncated due to excessive length. */
  truncated: boolean;
  /** The original, unmodified text (for logging purposes). */
  original: string;
}
 ...
 */
export function sanitizeIncomingMessage(raw: string): SanitizeResult {
  const original = raw;
...
export function logSuspiciousInput(
  ...
  const preview = original.length > 120
    ...
    : original;
  console.warn(
    `🚨 [Security] Prompt injection attempt detected | tenant=${tenantId} | phone=${phone} | preview="${preview}"`
  );
}
