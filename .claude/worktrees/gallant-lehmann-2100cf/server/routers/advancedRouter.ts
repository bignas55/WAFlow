...
import { conversations, agents, conversationAssignments, templates, agentMetrics } from "../../drizzle/schema.js";
...
export const advancedRouter = router({
  ...
  agents: protectedProcedure.query(async () => {
    ...
    const metrics = await db
      .select({ agentId: agentMetrics.agentId, avgScore: sql<number>`AVG(${agentMetrics.satisfactionScore})` })
      .from(agentMetrics)
      .groupBy(agentMetrics.agentId);
    const scoreMap = Object.fromEntries(metrics.map(m => [m.agentId, Number(m.avgScore ?? 0)]));
    return rows.map((a) => ({
      id: a.id,
      name: a.name,
      email: a.email,
      ...
      satisfactionScore: scoreMap[a.id] ?? null,
    }));
  }),
