...
export const appointmentsRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const rows = await db
        ...
        // Admin sees all; tenants see only their own appointments
        .where(isAdmin ? undefined : eq(appointments.tenantId, ctx.user.userId))
    ...
    .query(async ({ input, ctx }) => {
      ...
      const dayAppts = await db
        ...
        .where(and(
          ...
          eq(appointments.tenantId, ctx.user.userId),
        ));
  ...
  book: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const dayAppts = await db.select({ time: appointments.time, serviceId: appointments.serviceId })
        .from(appointments)
        .where(and(eq(appointments.date, input.date), eq(appointments.status, "scheduled"), eq(appointments.tenantId, tenantId)));
      ...
      for (const a of dayAppts) {
        ...
        const existEnd = existStart + dur;
        if (newStart < existEnd && newEnd > existStart) {
          throw new Error(`Booking conflict: the ${input.time} slot overlaps with an existing appointment at ${a.time}.`);
        }
      }
  ...
  services: protectedProcedure.query(async ({ ctx }) => {
    return db.select().from(services)
      .where(and(eq(services.tenantId, ctx.user.userId), eq(services.isActive, true)))
  ...
  sendInvoice: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!appt || !appt.customerPhone) throw new Error("Appointment not found");
    ...
    const rows = await db
      ...
      .where(
        isAdmin
          ...
          : and(eq(appointments.date, today), eq(appointments.tenantId, ctx.user.userId))
      )
