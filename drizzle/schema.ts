...
export const botConfig = mysqlTable("bot_config", {
  ...
  // ── Dual AI Model (Groq + Ollama Fallback) ───────────────────────────
  ...
  fallbackAiTimeout: int("fallback_ai_timeout").default(5000).notNull(),
  usedFallbackCount: int("used_fallback_count").default(0).notNull(),
  lastFallbackAt: datetime("last_fallback_at"),
...
export const systemSettings = mysqlTable("system_settings", {
  ...
  // Global AI Model overrides (if set, these override tenant's botConfig)
  ...
  // Metadata
  lastUpdatedBy: int("last_updated_by"), // admin user id who made the change
...
export const conversations = mysqlTable("conversations", {
  ...
  // AI model tracking
  modelUsed: varchar("model_used", { length: 50 }).default("groq"),
...
export const templates = mysqlTable("templates", {
  ...
  tenantActiveIdx: index("idx_templates_tenant_active_priority").on(t.tenantId, t.isActive, t.priority),
}));
...
export const knowledgeBase = mysqlTable("knowledge_base", {
  ...
  tenantActiveIdx: index("idx_kb_tenant_active_status").on(t.tenantId, t.isActive, t.status),
}));
...
export const customers = mysqlTable("customers", {
  ...
  tenantPhoneIdx: index("idx_customers_tenant_phone").on(t.tenantId, t.phoneNumber),
  tenantCreatedIdx: index("idx_customers_tenant_created").on(t.tenantId, t.createdAt),
}));
